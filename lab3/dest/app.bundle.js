...
var _storyBook = __webpack_require__(15);
...
var _mock = __webpack_require__(8);
...
function reducer(state, action) {
	switch (action.type) {
		...
		case 'BUY_GENERATOR':
			const generator = new _generator2.default(Object.assign({}, state.generators[action.payload.attribute]));
		...
		case 'CHECK_STORY':
			for (var x in state.story) {
				const checkStory = new _story2.default(Object.assign({}, state.story[x]));
...
const generator = exports.generator = {
	type: 'autonomous',
	name: 'Grandma',
	description: 'desc',
	rate: 10,
...
class Generator {
  ...
  */
	getCost() {
		...
		var cost = (this.baseCost * (1 + _constants2.default.growthRatio) ** this.quantity).toFixed(2);
		return Number(cost);
	}
  ...
  */
	generate() {
		var generate = this.rate * this.quantity;
		return generate;
	}
}
...
class Story {
  ...
  */
	isUnlockYet(value) {
		...
		return value >= this.triggeredAt ? true : false;
	}
